...
setwd("C:/Users/jgree/Downloads/Intro to R/Module10")
...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      selectInput(inputId = "subset_income", label = "Select Income Range", choices = c("<=50K", ">50K"), multiple = FALSE),
      # Interactive piece 2: inputId = "set_yaxis"
      selectInput(inputId = "set_yaxis", label = "Set Y Axis", choices = c("hours_per_week", "capital_loss")),
...
server <- function(input, output) {
  ...
  output$myfigure <- renderPlot(ggplot(create_subset()) +
                                  ...
                                  geom_boxplot(aes_string(x = "occupation", y = input$set_yaxis), fill = "pink") +
                                  theme_bw(18) +
